...
void smb_put_task(void *pvParameters);
...
void app_main(void)
{
	...
	/* Create SMB put Task */
	xTaskCreate(&smb_put_task, "SMB", 1024*8, NULL, 5, NULL);
...
#define SHUTTER "HTTP Request"
	...
	while(1) {
		...
		if (xQueueSend(xQueueRequest, &requestBuf, 10) != pdPASS) {
			ESP_LOGE(TAG, "xQueueSend xQueueRequest fail");
			continue;
		}

		// Receive responce from SMB
